...
/**
  ...
  */
cpp_int AutolykosPowScheme::getB(uint32_t nBits) {
    return q / decodeCompactBits(nBits);
}
...
cpp_int AutolykosPowScheme::genElement(int version,
    ...
    if (version == 1) {
        ...
        return hashModQ(concat({indexBytes, M, pk, m, w}));
    } else {
        // Autolykos v. 2: H(j|h|M)
...
void AutolykosPowScheme::validate(const Header& header) {
    ...
    if (header.version == 1) {
        ...
        if (hit >= b) {
            std::cout << "h(f) < b condition not met" << std::endl;
        } 
    }
}
...
std::optional<Header> AutolykosPowScheme::prove(const std::optional<Header>& parentOpt,
                                                ...
                                                long minNonce,
                                                long maxNonce) {
    auto [parentId, height] = derivedHeaderFields(parentOpt);
    ...
    // Assuming height is 4 bytes (uint32_t)
