...
void testPreviousHeightsRequiredForRecalculation() {
    ...
    std::vector<Height> result = difficultyAdjustment.previousHeightsRequiredForRecalculation(epochLength/2 +1, epochLength);
    printResult(epochLength/2 +1, epochLength, result);
    
    //2 Get the result from the method
    result = difficultyAdjustment.previousHeightsRequiredForRecalculation(epochLength * useLastEpochs, epochLength);
    printResult(epochLength * useLastEpochs , epochLength, result);
    
    //3 Get the result from the method
    result = difficultyAdjustment.previousHeightsRequiredForRecalculation(epochLength * useLastEpochs +2, epochLength);
    printResult(epochLength * useLastEpochs +2, epochLength, result);
}
